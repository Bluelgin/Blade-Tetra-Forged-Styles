...
> **Blade Tetra: Forged Styles** — 面向 Minecraft 1.20.1 的 Tetra × 拔刀剑：重锋模块化锻刀与战斗流派扩展。

![拔刀剑：百构流派封面](.github/assets/cover.jpg)

> 📖 想自己解读锻刀残页，或者直接查看完整答案？请前往[特殊刀与隐藏传承解锁 Wiki](docs/legacy-wiki/README.md)。答案默认折叠，不会意外剧透。

> ⚔️ 不知道该选哪种刀身，或者怎样稳定打出连段？请查看[战斗与连招指南](docs/COMBAT_GUIDE.zh-CN.md)。

玩家可以在 Tetra 工作台中组合刀身、刀柄、刀鞘、刀镡等部件，并由构造与材料自然决定武器外观、耐久、完整度、锻造需求和战斗流派。模组目前提供居合流、连舞流、断岳流三种路线，以及锻刀秘录、隐藏传承、动态材料纹理与可选的 Contract Blade 联动。
...
- Alpha 12 新增 23 种基于 Forge 通用锭标签的完整 Tetra 材料定义：
