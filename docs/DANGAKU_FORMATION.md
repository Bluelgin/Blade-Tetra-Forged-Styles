...
- 普通 Sweep：旧 `0.92x` 再乘 `0.68`，最终约 **`0.63x`**；
- Cleave：旧 `1.12x` 再乘 `0.95`，最终约 **`1.06x`**；
- 紧密敌群的断阵奖励：从约 `+10%` 下调为 **`+6%`**；
- 蓄力横扫 ratio：约 **`0.42x → 0.58x~0.68x`**，随蓄力和面板柔性变化；
- 当横扫与 SA 同时释放时，横扫伤害再乘 **`0.72`**，最高大约 **`0.49x`**。

SA 是主要爆发来源；断岳横扫的核心价值是大范围整阵、聚怪与制造下一刀/SA 的命中条件。
...
9. 长按 HUD 从 0% 向 100% 上升。
10. 约 2.2 秒进入断岳满蓄，保持约 0.25 秒宽限。
11. 继续按住后进度条单向下降，不重新循环到 100%。
12. 满蓄低面板范围约 8.5+，高面板不超过 12 格。
13. 大横扫最多处理 24 个合法目标，不能隔墙命中。
14. 普通/蓄力 Sweep 都能明显把敌群压向玩家前方收束区。
15. 高击退抗性的目标受到较弱位移。
16. 聚成至少 3 个目标后纵劈获得较小的断阵收益。
17. 大横扫不会 `forceHit/resetHit` 清掉已有 i-frame。
18. 潜行方向技、空中技仍保持原生路径。
19. 不新增永久 charge NBT，也不新增网络包。
