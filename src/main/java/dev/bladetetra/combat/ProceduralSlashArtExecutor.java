...
/**
 * Shared bounded projectile grammar used by generated Slash Arts.
 *
 * <p>The programmatic legacy-fusion fallback and the player-authored Tetra
 * Slash Art system deliberately share this executor. Source SlashArts are never
 * invoked here: the caller owns damage, timing and topology.</p>
 */
final class ProceduralSlashArtExecutor {
    ...
    static void execute(ServerPlayer player, Vec3 forward,
        ...
        Vec3 aim = forward.lengthSqr() < 1.0E-8D
                ? player.getLookAngle() : forward.normalize();
    ...
    static void spawnDrive(ServerPlayer player, Vec3 direction,
        ...
        Vec3 normalized = direction.lengthSqr() < 1.0E-8D
                ? player.getLookAngle().normalize() : direction.normalize();
