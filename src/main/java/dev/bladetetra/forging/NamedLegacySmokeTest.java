        ...
        try {
            ...
            for (var kind : NamedLegacyCatalog.values()) {
                ...
                for (String part : java.util.List.of("saya", "tsuba")) {
                    ...
                    if (definition == null) throw new IllegalStateException("Missing schematic " + kind.schematic(part));
                    var schematic = new ConfigSchematic(definition);
                    var requirement = new dev.bladetetra.compat.LegacyPatternRequirement(kind.id(), part);
                    var locked = new se.mickelus.tetra.module.schematic.CraftingContext(player.level(),
                            player.blockPosition(), player.level().getBlockState(player.blockPosition()),
                            player, stack, "slashblade/" + part, new ResourceLocation[0]);
                    var unlocked = new se.mickelus.tetra.module.schematic.CraftingContext(player.level(),
                            player.blockPosition(), player.level().getBlockState(player.blockPosition()),
                    ...
                    ItemStack[] materials = {
                            new ItemStack(ForgeRegistries.ITEMS.getValue(new ResourceLocation("slashblade:proudsoul_ingot"))),
                            new ItemStack(ForgeRegistries.ITEMS.getValue(new ResourceLocation(kind.material())))
                    };
                    ...
                    stack = schematic.applyUpgrade(stack, materials, false, "slashblade/" + part, player);
                    if (!part.equals("tsuba") && NamedLegacyParts.fromStack(stack).completeSet() != null)
                ...
                if (!kind.equals(NamedLegacyParts.fromStack(stack).completeSet()))
                    throw new IllegalStateException("Incomplete crafted set " + kind.id() + ": " + stack.getTag());
